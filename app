...
import numpy as np
from tensorflow.keras.preprocessing.image import img_to_array  # Explicit import
...
import cv2
from torchcam.methods import GradCAM  # Replaced pytorch_grad_cam
...
def predict_class(img):
    try:
        ...
        img_array = img_to_array(img)  # Fixed with explicit import
...
def generate_heatmap(model, img_tensor, pred_class):
    cam_extractor = GradCAM(model, target_layer="layer4[-1]")  # TorchCAM syntax
    with torch.no_grad():
...
def main():
    ...
    if uploaded_file:
        ...
        with tab2:
            ...
            translator = deepl.Translator("YOUR_DEEPL_API_KEY")  # Replace with your DeepL API key
            ...
            with st.spinner("Analyzing..."):
                ...
                disease_trans = translator.translate_text(disease.replace("___", " - "), target_lang=languages[lang]).text
                species_trans = translator.translate_text(species, target_lang=languages[lang]).text
